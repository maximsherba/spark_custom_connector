...
class PostgresScan(connectionProperties: ConnectionProperties) extends Scan with Batch {
  ...
  override def planInputPartitions(): Array[InputPartition] = {
    if (connectionProperties.partitionSize > 10) {
      val connection = DriverManager.getConnection(connectionProperties.url,
  ...
  private  def partitions(partitionSize: Int, count: Int): Array[InputPartition] = {
    partitionList(partitionSize, count).reverse.toArray
  }

  @tailrec
  private def partitionList(partitionSize: Int, count: Int, offset: Int = 0, acc: List[PostgresPartition] = Nil): List[InputPartition] = {
    if (offset >= count) acc
    else partitionList(partitionSize, count, offset + partitionSize, new PostgresPartition(offset, partitionSize) :: acc)
  }
}
